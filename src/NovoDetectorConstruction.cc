...
void NovoDetectorConstruction::DefineMaterials()
{
  ...
  fEJ276MPT->AddConstProperty("FASTTIMECONSTANT", 13.*ns); // Decay time
  //fEJ276MPT->AddConstProperty("FASTTIMECONSTANT", 1.1*ns); // Decay time
  ...
  fEJ276MPT->AddConstProperty("SLOWSCINTILLATIONRISETIME", 35.*ns); //Rise time
...
G4VPhysicalVolume* NovoDetectorConstruction::ConstructDetector()
{
  ...
  fBSDetectorPhys = new G4PVPlacement(rotationMatrix,G4ThreeVector(BS_xpos,0,0),fBSDetectorLog,"BS_detector",fWorldLog,false,0);
